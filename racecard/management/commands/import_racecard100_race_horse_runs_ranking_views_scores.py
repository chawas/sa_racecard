...
class Command(BaseCommand):
    ...
    def analyze_jockey_trainer_combination(self, html_row):
        ...
        Each row contains two horses (each with 9 columns of data)
        Structure: 
            <td>Horse1_No</td><td>Horse1_Trainer</td><td>Horse1_Jockey</td><td>Horse1_Data1</td>...<td>Horse1_Data6</td>
            <td>Horse2_No</td><td>Horse2_Trainer</td><td>Horse2_Jockey</td><td>Horse2_Data1</td>...<td>Horse2_Data6</td>
        """
        ...
        # Check if we have enough cells for at least one horse (9 cells per horse)
        if len(cell_texts) < 9:
            self.stdout.write(f"❌ Not enough cells for a complete horse: {len(cell_texts)}")
            return results
        
        # Parse horses in pairs - each horse uses 9 cells
        horse_count = len(cell_texts) // 9
        extra_cells = len(cell_texts) % 9
        
        if extra_cells > 0:
            self.stdout.write(f"⚠️  Warning: Row has {len(cell_texts)} cells ({extra_cells} extra)")
        ...
        for horse_index in range(horse_count):
            ...
            try:
                ...
                # The next 6 cells contain numeric data
                # Based on your screenshot, the structure appears to be:
                # [3]: ? (possibly starts/runs)
                # [4]: 1st places
                # [5]: 2nd places  
                # [6]: 3rd places
                # [7]: Win percentage or total wins?
                # [8]: Place percentage or something else?
                ...
                # Calculate score (adjust this formula based on actual meaning of the columns)
                score = (
                ...
                self.stdout.write(f"❌ Error parsing horse {horse_index + 1} at cells {start_idx}-{end_idx}: {e}")
                continue

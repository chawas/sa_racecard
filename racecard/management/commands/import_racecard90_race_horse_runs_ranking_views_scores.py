...
class Command(BaseCommand):
   ...
   # -------------------------
    ...
    def _parse_horses(self, soup, race, update_existing: bool):
        ...
        self.stdout.write(f"J-T analysis data values: {jt_analysis_data}")
        
        # Store jockey-trainer analysis in race for later use
        race.jt_analysis_data = jt_analysis_data
        self.stdout.write(f"✅ Stored J-T data in race object: {hasattr(race, 'jt_analysis_data')}")
    ...
    def _parse_jockey_trainer_table(self, soup):
        ...
        self.stdout.write("Looking for table with structure: [HorseNo, Trainer, Jockey, Rns, 1st, 2nd, 3rd, Win%, PLC%]")
        
        # Debug: Show all tables and their structure
        all_tables = soup.find_all('table')
        self.stdout.write(f"Found {len(all_tables)} total tables in HTML")
        ...
        for i, table in enumerate(all_tables):
            ...
            if has_jockey and has_trainer:
                ...
                for j, row in enumerate(table.find_all('tr')):
                    ...
                    # Parse the J-T data (now returns a list of results for each column)
    ...
    # -------------------------
    # Jockey-Trainer Analysis Functions (updated)
    # -------------------------
    ...
    def calculate_horse_score(self, horse):
        ...
        if hasattr(horse.race, 'jt_analysis_data'):
            self.stdout.write(f"Race has J-T data attribute: {hasattr(horse.race, 'jt_analysis_data')}")
            self.stdout.write(f"J-T data keys: {list(horse.race.jt_analysis_data.keys())}")
            
            if horse.horse_no in horse.race.jt_analysis_data:
                ...
                jockey = jt_data.get('jockey', jockey)
                trainer = jt_data.get('trainer', trainer)
                self.stdout.write(f"✅ Using J-T data from race: Score={jt_score}")
            else:
                self.stdout.write(f"❌ Horse {horse.horse_no} not found in race J-T data")
                self.stdout.write(f"Available horse numbers: {list(horse.race.jt_analysis_data.keys())}")
        else:
            self.stdout.write("❌ No J-T data found in race object")

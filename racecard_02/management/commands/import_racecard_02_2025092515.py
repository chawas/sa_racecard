...
class Command(BaseCommand):
    ...
    def _calculate_horse_scores(self, race):
        """Calculate horse scores AND save rankings to database"""
        from racecard_02.models import Horse, HorseScore, Ranking
        from django.db import transaction
        
        self.stdout.write(f"\n📊 Calculating scores and rankings for Race {race.race_no}...")
        ...
        for horse in horses:
            try:
                self.stdout.write(f"    🐎 Scoring {horse.horse_name}...")
                
                result = self.scoring_service.create_score_record(horse, race)
                
                if result:
                    score_record, created = result
                    status = "Created" if created else "Updated"
        ...
        # Step 2: Save rankings to database
        if horse_scores:
            self._save_rankings_to_database(race, horse_scores)
        else:
            self.stdout.write("    ⚠️ No horse scores to rank")

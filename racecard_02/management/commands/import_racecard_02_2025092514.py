...
class Command(BaseCommand):
    ...
    def calculate_rankings(self, ranking_date=None, race_id=None):
        """
        Calculate and save rankings based on HorseScore data
        """
        from racecard_02.models import Race, HorseScore, Ranking
        from django.db import transaction
        from django.utils import timezone
        
        self.stdout.write("📊 Calculating horse rankings...")
        
        # Get races to rank
        if race_id:
            races = Race.objects.filter(id=race_id)
            self.stdout.write(f"Calculating rankings for race ID: {race_id}")
        elif ranking_date:
            try:
                target_date = timezone.datetime.strptime(ranking_date, '%Y-%m-%d').date()
                races = Race.objects.filter(race_date=target_date)
                self.stdout.write(f"Calculating rankings for date: {target_date}")
            except ValueError:
                self.stderr.write("Invalid date format. Use YYYY-MM-DD")
                return
        else:
            # Get recent races (last 30 days) that have horse scores
            recent_date = timezone.now().date() - timezone.timedelta(days=30)
            races = Race.objects.filter(
                race_date__gte=recent_date,
                horsescore__isnull=False
            ).distinct()
            self.stdout.write(f"Calculating rankings for recent races since {recent_date}")
        
        total_rankings_created = 0
        
        for race in races:
            rankings_created = self.calculate_race_rankings(race)
            total_rankings_created += rankings_created
        
        self.stdout.write(
            self.style.SUCCESS(
                f"✅ Successfully created/updated {total_rankings_created} rankings across {races.count()} races"
            )
        )
    
    def calculate_race_rankings(self, race):
        """
        Calculate rankings for a specific race
        """
        ...
        if not horse_scores.exists():
            self.stdout.write(f"❌ No horse scores found for race {race}")
            return 0
        ...
        with transaction.atomic():
            ...
            if deleted_count:
                self.stdout.write(f"🗑️ Cleared {deleted_count} existing rankings for race {race}")
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"✅ Created {rankings_created} rankings for {race.race_field} R{race.race_no} ({race.race_date})"
                )
            )
        
        return rankings_created

    def display_rankings(self, race_id=None, date_str=None):
        """
        Display rankings for verification
        """
        from racecard_02.models import Ranking
        from django.utils import timezone
        
        if race_id:
            rankings = Ranking.objects.filter(race_id=race_id).select_related('horse', 'race')
        elif date_str:
            target_date = timezone.datetime.strptime(date_str, '%Y-%m-%d').date()
            rankings = Ranking.objects.filter(race__race_date=target_date).select_related('horse', 'race')
        else:
            rankings = Ranking.objects.all().select_related('horse', 'race')[:10]  # Last 10
        
        self.stdout.write("\n=== CURRENT RANKINGS ===")
        for ranking in rankings:
            self.stdout.write(
                f"Rank {ranking.rank}: {ranking.horse.horse_name} "
                f"({ranking.race.race_date} R{ranking.race.race_no}) - "
                f"Score: {ranking.overall_score:.1f}"
            )
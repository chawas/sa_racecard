...
class Command(BaseCommand):
        ...
        try:
            if filename:
                ...
                if calculate_rankings and races:
                    for race in races:
                        self.calculate_race_rankings(race)
    ...
    def _save_rankings_to_database(self, race, horse_scores):
        """Save calculated scores to Ranking table"""
        from racecard_02.models import Ranking
        from django.db import transaction
        
        self.stdout.write(f"    💾 Saving rankings to database...")
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write(f"    ✅ Saved {rankings_created} rankings to database")
                
        except Exception as e:
            self.stdout.write(f"    ❌ Error saving rankings to database: {e}")
            import traceback
            self.stdout.write(f"    Traceback: {traceback.format_exc()}")

    def _display_simple_rankings(self, race):
        """Display rankings from database AND calculated scores"""
        from racecard_02.models import HorseScore, Ranking
        
        self.stdout.write(f"\n🔍 Checking rankings for Race {race.race_no}...")
        
        # Check database rankings
        db_rankings = Ranking.objects.filter(race=race).select_related('horse').order_by('rank')
        self.stdout.write(f"📀 Rankings in DATABASE: {db_rankings.count()}")
        
        # Check calculated rankings
        calculated_scores = HorseScore.objects.filter(race=race).select_related('horse').order_by('-overall_score')
        self.stdout.write(f"🧮 Calculated scores: {calculated_scores.count()}")
        
        # Display database rankings if they exist
        if db_rankings.exists():
            self.stdout.write("\n" + "="*60)
            self.stdout.write(f"🏆 DATABASE RANKINGS - Race {race.race_no}")
            ...
            for ranking in db_rankings:
                ...
                self.stdout.write(
                    ...
                    f"{ranking.overall_score:<6.1f} "
                    f"{getattr(horse, 'speed_rating', 0):<6} "
                    f"{getattr(horse, 'jt_score', 50):<5} "
                )
        
        # Also display calculated rankings for comparison
        if calculated_scores.exists():
            self.stdout.write("\n" + "="*60)
            self.stdout.write(f"🧮 CALCULATED SCORES - Race {race.race_no}")
            self.stdout.write("="*60)
            ...
            for position, score in enumerate(calculated_scores, 1):
                horse = score.horse
                self.stdout.write(
                    f"{position:<3} "
                    f"{horse.horse_no:<3} "
    ...
    def _calculate_horse_scores(self, race):
        """SIMPLE VERSION - Table now exists, so this will work"""
        from racecard_02.models import HorseScore, Ranking
        from django.db import transaction
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write("✅ Cleared existing rankings")
                ...
                for rank, score in enumerate(sorted_scores, 1):
                    ...
                    if is_magic_tip:
                        self.stdout.write(f"✨ Magic Tip #{rank}: {score.horse.horse_name} = {final_score:.1f}")
                
                self.stdout.write(f"✅ Saved {len(sorted_scores)} rankings to database!")
                
        except Exception as e:
            self.stdout.write(f"❌ Error saving to database: {e}")
            # Fallback: just calculate and display
            self._display_calculated_rankings(race, horse_scores, magic_tips)
            return
        
        # Display the results
        self._display_final_rankings(race)

    def _display_final_rankings(self, race):
        """Display rankings from the database"""
        from racecard_02.models import Ranking
        
        rankings = Ranking.objects.filter(race=race).select_related('horse').order_by('rank')
        
        self.stdout.write(f"\n🏆 FINAL RANKINGS WITH MAGIC TIPS - Race {race.race_no}")
        self.stdout.write("="*70)
        self.stdout.write("Rank  Horse No  Horse Name          Score  Magic Tip")
        self.stdout.write("-"*70)
        
        for ranking in rankings:
            star = "✨" if ranking.is_magic_tip else ""
            self.stdout.write(f"{ranking.rank:2d}    {ranking.horse.horse_no:2d}      {ranking.horse.horse_name:<18}  {ranking.adjusted_score:5.1f}  {star}")
        
        self.stdout.write("="*70)
        
        # Show Magic Tips summary
        magic_tips_count = rankings.filter(is_magic_tip=True).count()
        self.stdout.write(f"🎯 Magic Tips applied to {magic_tips_count} horses (40% boost)")
    ...
    def _display_calculated_rankings(self, race, horse_scores, magic_tips):
        ...
        for rank, score in enumerate(sorted_scores, 1):
            is_magic_tip = score.horse.horse_no in magic_tips
            final_score = (score.overall_score * 0.6) + (100 * 0.4) if is_magic_tip else score.overall_score

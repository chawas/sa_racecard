...
class Command(BaseCommand):
    ...
    def _import_runs_for_all_horses(self):
        ...
        try:
            ...
            for horse in horses:
                self.stdout.write(f"\n    📥 Processing {horse.horse_name} (#{horse.horse_no})...")
                ...
                # Save runs to database USING HORSE OBJECT
                for run_data in runs_data:
                    try:
                        run_date = datetime.strptime(run_data['date'], '%Y-%m-%d').date()
                        
                        # ✅ Use horse object as foreign key
                        run, created = Run.objects.update_or_create(
                            horse=horse,  # Use the horse object
                            date=run_date,
                            defaults={
                ...
                self.stdout.write(f"    ✅ Imported {len(runs_data)} runs for {horse.horse_name}")
                    
        except Exception as e:
            self.stdout.write(f"    ❌ Error in run import process: {e}")
        
        self.stdout.write(f"\n🎯 Total runs imported: {total_imported}")
    ...
    def _display_runs_table(self, runs_data, horse_name):
        ...
        if not runs_data:
            self.stdout.write(f"    No runs to display for {horse_name}")
            return
        
        self.stdout.write(f"\n    🏇 RUN HISTORY FOR {horse_name.upper()}")
        self.stdout.write("    ┌──────────┬──────────┬──────┬────┬──────┬────────┬──────────┬──────────┐")
        self.stdout.write("    │   Date   │  Track   │ Dist │ Pos│Margin│ Weight │  Jockey  │   Time   │")
        self.stdout.write("    ├──────────┼──────────┼──────┼────┼──────┼────────┼──────────┼──────────┤")
        ...
        for run in runs_data:
            ...
            time_display = f"{run['time_seconds']}" if run['time_seconds'] else "N/A"
            
            self.stdout.write(f"    │ {date_display:8} │ {track_display:8} │ {dist_display:4} │ {pos_display:2} │ {margin_display:4} │ {weight_display:6} │ {jockey_display:8} │ {time_display:8} │")
    ...
    def _get_horse_past_performance_compartment(self, horse):
        """Find the HTML compartment for PAST PERFORMANCE (not predicted finish)"""
        try:
            ...
            for table_idx, table in enumerate(all_tables):
                ...
                for row in run_rows[:2]:  # Check first 2 rows
                    cells = row.find_all('td')
                    if len(cells) > 0:
            ...
            self.stdout.write(f"    ❌ Error finding past performance compartment: {e}")
            return None
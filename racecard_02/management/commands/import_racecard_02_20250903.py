...
# def calculate_speed_score(self, run_analysis):  # DELETE THIS
    ...
    def calculate_trainer_score(self, trainer_name):
        """
        Calculate trainer score based on trainer performance statistics
        """
        if not trainer_name:
            return 50
            
        try:
            # If you have trainer statistics in your database, use them:
            # from racecard_02.models import TrainerStats
            # try:
            #     stats = TrainerStats.objects.get(trainer_name=trainer_name)
            #     return min(100, max(0, stats.win_percentage * 100))
            # except TrainerStats.DoesNotExist:
            #     pass
            
            # Fallback to simple name-based scoring

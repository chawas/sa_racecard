...
class ScoringService:
    ...
    def create_score_record(self, horse: Horse, race: Race) -> Tuple[HorseScore, bool]:
        ...
        score_record, created = HorseScore.objects.update_or_create(
            ...
            defaults={
                ...
                'blinkers_score': scores['blinkers'],  # FLOAT value
                
                # Store raw values for reference
                'speed_score': getattr(horse, 'speed_rating', 0),
                ...
                'weight_value': 0.0,  # Float value
                'draw_value': float(horse.horse_no),  # Float value
                'blinkers_value': blinkers_bool,  # BOOLEAN value (True/False)
    ...
    def _calculate_magic_tips_boost(self, horse: Horse, base_score: float, is_magic_tip: bool) -> float:
        """Calculate Magic Tips boost"""
        if not is_magic_tip:
            return 0.0
        
        # Magic Tips get a significant boost
        boost = 25.0  # Base boost for being a Magic Tip
        
        # Additional boost based on horse quality
        if base_score >= 70:
            boost += 15.0  # High-quality Magic Tip
        elif base_score >= 60:
            boost += 10.0  # Medium-quality Magic Tip
        
        self._debug(f"   🎯 Magic Tip boost: {boost:.1f}")
        return boost
    ...
    # KEEP ALL YOUR EXISTING SCORING METHODS EXACTLY AS THEY ARE
    ...
    def _calculate_overall_score(self, scores: Dict[str, float]) -> float:
        ...
        weights = {
            'speed': 0.45,       # 20% - Speed rating and current form
            'form': 0.40,        # 15% - Current vs best form
            'class': 0.40,       # 15% - Class suitability
            'consistency': 0.12, # 12% - Consistency (J-T combo)
            'value': 0.10,       # 10% - Odds value
            'physical': 0.30,    # 10% - Physical condition
            'intangible': 0.08,  # 8%  - Intangible factors
            'draw': 0.15,        # 5%  - Draw position
            'weight': 0.35,      # 5%  - Weight (when available)
        }
